---
title: "Análisis de Sobrerrepresentación con ortólogos de humano"
...
path_ids <- str_split_i(hsa_pathways$pathway, "hsa", 2)
path_descriptions <- str_split_i(hsa_pathways$description, " -", 1)
hsa_pathways$pathway <- path_ids
hsa_pathways$description <- path_descriptions
```
...
bottom_genes <- bottom$HGNC_Symbol

writeLines(as.character(bottom_genes), con = "72_48_bottom_genes_list.txt", sep = "\n")

kegg_genes <- bitr(bottom_genes, "SYMBOL", "ENTREZID", org.Hs.eg.db)$ENTREZID
```
...
kegg_genes <- bitr(top_genes, "SYMBOL", "ENTREZID", org.Hs.eg.db)$ENTREZID
```
